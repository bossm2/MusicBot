...
slimbot2.on('message', message => {
  ...
  slimbot2.sendMessage(message.chat.id, 'Message received', optionalParams);
});

// Because each inline keyboard button has callback data, you can listen for the callback data and do something with them

slimbot2.on('callback_query', query => {
  if (query.data === 'hello') {
    slimbot2.sendMessage(query.message.chat.id, 'Hello to you too!');
  }
});
