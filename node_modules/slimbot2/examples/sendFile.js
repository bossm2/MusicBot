...
let fileUpload = fs.createReadStream('C:\\Users\\mypc\\Desktop\\musicbot\\node_modules\\slimbot2\\examples\\bulb.png');
...
slimbot2.on('message', (message) => {
  ...
  console.log(message);
  slimbot2.sendPhoto(message.chat.id, fileId).then(message => {
    console.log(message.result.photo);
  });

  // Method 2: Using HTTP URL
  slimbot2.sendPhoto(message.chat.id, fileURL).then(message => {
    console.log(message.result.photo);
  });

  // Method 3: Uploading a file to Telegram's servers
  slimbot2.sendPhoto(message.chat.id, fileUpload).then(message => {
    console.log(message.result.photo);
  });
});
